...
class checkers:
    def __init__(self):
        ...
        self.opponent=None
        self.difficulty=3
        self.figure_counter=[24,0]
        
    def setup(self):
        print("\033[0;32m\t\t\t\t\tCHECKERS GAME\033[0m")
        a=input("\033[0;34mPlay against AI? (y/n): \033[0m").lower()
        if a=='n':
    ...
    def palyer_valid_moves(self, x, y, player=None, opponent=None, board=None):
        ...
        directions=[]
        if not is_king: #for original ones
            directions = [(player['direction'], -1), (player['direction'], 1)]
        else: #king directions
    ...
    def make_move(self,x,y,nx,ny,is_capture,player=None,opponent=None,board=None):
        if player is None:
            player=self.current_player
            opponent=self.opponent
            board=self.board
                ...
                if capture_moves:
                    ...
                    while True:
                        ...
                        if a.isdigit() and 0<int(a)<=len(moves):
                            ...
                            result=self.make_move(x, y, nx, ny, is_capture)
                            break
                        print("Invalid move")      
                else:
                    while True:
